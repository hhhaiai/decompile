...
def rc4(Key:bytes, inData:bytes) -> bytes:
    Buf = b""
    S = [_ for _ in range(256)]
    K = (Key * int(256 / len(Key) + 1))[:256]
    j = 0
    for i in range(256):
        j = (S[i] + K[i] + j) % 256
        S[i], S[j] = S[j], S[i]
    ...
    for x in range(len(inData)):
        ...
        j = (j + S[i]) % 256
        S[i], S[j] = S[j], S[i]
        Buf += int.to_bytes(S[(S[j] + S[i]) % 256] ^ inData[x], 1, 'big')
    return Buf
...
def KeygenSN(LicenseSerial, MachineID):
    ...
    Z1 = int(time.time()+100000000) ^ 0x56739ACD
    s = sum(map(lambda x:int(x, 16), "%x" % Z1)) % 10
    return "%dZ%d%d" % (Z0, Z1, s)

def ParsePost(buf:bytes):
...
def DecodeRc4Str(buf:str):
    buf = binascii.unhexlify(buf.encode())
